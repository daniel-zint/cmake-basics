	...
	void dmoGPU( thrust::device_vector<float2>& points_d, DmoMesh<true> &dmoMesh1_, const MetricT& metric ) {
		for( auto colorIt = dmoMesh1_.colorOffset_.begin(); colorIt != dmoMesh1_.colorOffset_.end() - 1; ++colorIt ) {
			...
			optimizeHierarchical << <nBlocks, N_THREADS >> > (
				points_d.data().get(),
				dmoMesh1_.coloredVertexIDs.d().data().get(),
				*colorIt,
				dmoMesh1_.vertices.d().data().get(),
				dmoMesh1_.oneRingVec.d().data().get(),
				metric
				);
		}
	}
...
template void DMO::dmoGPU<>( thrust::device_vector<float2>&, DMO::DmoMesh<true>&, const metric& );
