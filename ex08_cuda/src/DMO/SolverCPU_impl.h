...
namespace DMO
{
	...
	void optimizeHierarchical( const int cid, std::vector<float2>& points, const std::vector<int>& coloredVertexIDs, const int cOff, const std::vector<DMO::DmoVertex>& vertices, const std::vector<int>& oneRingVec, const MetricT metric ) {
		...
		// min/max search + loading oneRing
		{
			...
			for( int k = 0; k < v.oneRingSize - 1; ++k ) {
				...
				float2 dist = {
					std::abs( points[v.idx].x - vo.x ),
					std::abs( points[v.idx].y - vo.y )
				};
		...
		for( int depth = 0; depth < DMO::DEPTH; ++depth ) {
			...
			for( int i = 0; i < DMO::NQ; ++i ) {
				for( int j = 0; j < DMO::NQ; ++j ) {
					float2 p = {
						DMO::AFFINE_FACTOR * ( i * aabbMin.x + ( DMO::NQ - 1 - i ) * aabbMax.x ),
	...
	void dmoCPU( std::vector<float2>& points, DmoMesh<false>& dmoMesh1_, const MetricT& metric ) {
		...
		#pragma omp parallel for
			for( int i = 0; i < nBlocks; ++i ) {
				optimizeHierarchical(
					i,
					points,
					dmoMesh1_.coloredVertexIDs.h(),
					*colorIt,
					dmoMesh1_.vertices.h(),
					dmoMesh1_.oneRingVec.h(),
					metric
				);
			}
		}
...
template void DMO::dmoCPU<>( std::vector<float2>&, DMO::DmoMesh<false>&, const metric& );
